...
void CXhTreeCtrl::OnBegindrag(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = 0;
	...
	//调用回调函数判断拖放源是否合法,如果FireIsValidDragS为空，认为不能拖放
...
void CXhTreeCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if(m_bDragging && nFlags&MK_LBUTTON)
	{
		...
		if(point.x<rect.left+20)
			m_hItemDragD = NULL;*/
		//当条目被拖拽至树外时，目标拖拽项设置为源拖拽项 wht 15-08-20
		//设置为根节点，可能出现选中项不能改变的问题,暂未找到根本原因
...
BOOL CXhTreeCtrl::DeleteAllSonItems(HTREEITEM hParentItem)
{
	if(hParentItem==NULL)
		return FALSE;
	m_bSelectionComplete=FALSE;	//设为FALSE删除项时不发送TVN_SELCHANGED消息至父窗口 wht 15-08-20
