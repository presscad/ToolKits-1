...
// Attributes
protected:
	long m_liCurrWorkItem;
	BOOL m_bLeftBtnDown, m_bRightBtnDown, m_bLeftBtnOut;	
	void RevokeComboMenuItemSelected(bool revokeCmdExecute=false);
	DWORD DoSingleClick();
	void DrawButtonAspect(CDC* pDC);

// Operations
public:
    DWORD (*FireClickComboMenu)(CWnd* pParentWnd, CPoint point);
	void NotifyComboMenuClicked(UINT idCmdMenuItem,const char* szCmdMenuName);
