...
VersionServiceSoapProxy* CVersionServer::GetValidServer()
{
	...
	if(g_sServerURL[0]==0)
		...
		//strcpy(g_sServerURL,"http://localhost/VersionService/VersionService.asmx");	//www.xerofox.com
...
int CVersionServer::LoginUser(void* pEndUser)
{
	...
	if(m_pServer==NULL)
		return 0;
	_VersionServ__LoginByByteArr inputPara;
	...
	_VersionServ__LoginByByteArrResponse response;
...
bool CVersionServer::QueryObjects(const char* clsName,char* xmlscope,BUFFER_IO* pExterLocalBuff)
{
	...
	if(m_pServer==NULL)
		return false;
	_VersionServ__QueryObjects inputPara;
	...
	_VersionServ__QueryObjectsResponse response;
...
bool CVersionServer::DownloadObjects(const char* clsName, int* idarr, int countOfObj/*=1*/,UINT serialize/*=0*/,BUFFER_IO* pExterLocalBuff/*=NULL*/)
{
	...
	if(m_pServer==NULL)
		return false;
	VersionServ__ArrayOfInt arrayOfInt;
	arrayOfInt.int_=idarr;
	arrayOfInt.__sizeint_=countOfObj;
	_VersionServ__DownloadObjects inputPara;
	...
	if(strcmp(clsName,"TempAuthCard")==0)
		...
		return false;
	_VersionServ__DownloadObjectsResponse response;
