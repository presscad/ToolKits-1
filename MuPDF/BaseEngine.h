...
enum RenderTarget { View, Print, Export };
...
enum PageElementType { Link, Image, Comment };
...
public:
	...
	DWORD m_dwRawBitsSize; 
	RenderedBitmapEx(){m_lpRawBits=NULL;m_nWidth=m_nHeight=0;m_nEffByteWidth=0;m_uBitcount=0;m_dwRawBitsSize=0;}
	~RenderedBitmapEx() { 
		if(m_lpRawBits!=NULL)
		{
			delete[] m_lpRawBits;
...
class RenderedBitmap {
  ...
  public:
    ...
    bool StretchDIBits(HDC hdc, RectI target) const {
        ...
        bool ok = StretchBlt(hdc, target.x, target.y, target.dx, target.dy, bmpDC, 0, 0, size.dx, size.dy, SRCCOPY)>0;
...
struct PageAnnotation {
    ...
    PageAnnotation() : type(PageAnnotType::AnnotType_None), pageNo(-1) {}
...
class BaseEngine {
  public:
	...
	//获取当前页内容类型 
	const static BYTE PAGE_TYPE_UNINITIALIZED			= 0;	//未初始化
	const static BYTE PAGE_TYPE_PDF_IMAGE				= 1;	//pdf图像
	const static BYTE PAGE_TYPE_PDF_STROKE				= 2;	//pdf笔画
	const static BYTE PAGE_TYPE_PDF_IMAGE_AND_STROKE	= 3;	//pdf图像+笔画
	virtual BYTE GetPageType(int pageNo)=0;
	...
	virtual RenderedBitmapEx* RenderBitmapEx(int pageNo, float zoom, int rotation,
											RectD* pageRect = nullptr, /* if nullptr: defaults to the page's mediabox */
											RenderTarget target = RenderTarget::View,
											AbortCookie** cookie_out = nullptr) = 0;
