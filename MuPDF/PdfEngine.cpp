...
//从libmupdf.dll中导出常量数据异常，在这里重新定义常量 wht 18-06-19
...
static RenderedBitmapEx* new_rendered_fz_pixmap_ex(fz_context* ctx, fz_pixmap* pixmap) {
    int paletteSize = 0;
    bool hasPalette = false;
    ...
    if (bmpData && pixmap->n == 4 && pixmap->colorspace == fz_device_rgb(ctx)) {
        unsigned char* dest = bmpData;
        unsigned char* source = pixmap->samples;
        palette = (uint32_t*)bmi.Get()->bmiColors;
        BYTE grayIdxs[256] = {0};
	...
	//-h,表示从上到下,定位点在左上角
	// h,表示从下向上,定位点在左下角
	RenderedBitmapEx *pBitMap=new RenderedBitmapEx();
	pBitMap->m_nHeight=h;
	pBitMap->m_uBitcount = hasPalette ? 8 : 32;
	if(hasPalette)
		pBitMap->m_nWidth=rows8;
	else
		pBitMap->m_nWidth=((rows8/4+3)/4)*4;
	pBitMap->m_nEffByteWidth=((pBitMap->m_nWidth*3+3)/4)*4;
	DWORD dwCount=pBitMap->m_nEffByteWidth*pBitMap->m_nHeight;
	pBitMap->m_dwRawBitsSize=dwCount;
	pBitMap->m_lpRawBits=new BYTE[dwCount];
	unsigned char* lpBits=hasPalette ? bmpData : bgrPixmap->samples;
	RGBQUAD crPixelColor;
	crPixelColor.rgbReserved=0;
	//int nPaletteCount=0;
	//if(hasPalette)
	//	nPaletteCount=paletteSize*4;
	BYTE ciPixelBytes=pBitMap->m_uBitcount/8;
	...
	for(int rowi=0;rowi<h;rowi++)
	{
		for(int i=0;i<rows8;i++)
		{
			...
			RGBQUAD crPixelColor;
			crPixelColor.rgbReserved=0;
			if(pBitMap->m_uBitcount==1)
			{	//单色图
				int ibyte=rowi*rows8+i/8;
				if(ibyte&byteConstArr[i%8])
					crPixelColor.rgbBlue=crPixelColor.rgbGreen=crPixelColor.rgbRed=255;
			}
			else if(pBitMap->m_uBitcount==4)
			{	//16色图
				int ibyte=rowi*rows8+i/2;
				if(i%2==0)
					colorindex=(lpBits[ibyte]&0xf0)>>4;
				else
					colorindex=lpBits[ibyte]&0x0f;
				crPixelColor.rgbBlue=(LOBYTE(palette[colorindex]));
				crPixelColor.rgbGreen=(LOBYTE(((WORD)(palette[colorindex])) >> 8));
				crPixelColor.rgbRed=(LOBYTE((palette[colorindex])>>16));
			}
			else if(pBitMap->m_uBitcount==8)
			{	//256色图
				int ibyte=rowi*rows8+i;
				colorindex=lpBits[ibyte];
				crPixelColor.rgbBlue=(LOBYTE(palette[colorindex]));
				crPixelColor.rgbGreen=(LOBYTE(((WORD)(palette[colorindex])) >> 8));
				crPixelColor.rgbRed=(LOBYTE((palette[colorindex])>>16));
			}
			else if(pBitMap->m_uBitcount==24||pBitMap->m_uBitcount==32)
			{	//真彩24位或32位
				if(i%ciPixelBytes!=0)
					continue;
				int ibyte=rowi*rows8+i*ciPixelBytes;
				memcpy(&crPixelColor,&lpBits[ibyte],ciPixelBytes);
			}
			if(hasPalette)
				memcpy(&pBitMap->m_lpRawBits[rowi*pBitMap->m_nEffByteWidth+i*3],&crPixelColor,3);
			else
				memcpy(&pBitMap->m_lpRawBits[rowi*pBitMap->m_nEffByteWidth+(i/ciPixelBytes)*3],&crPixelColor,3);
		}
	}
...
static Vec<PageAnnotation> fz_get_user_page_annots(Vec<PageAnnotation>& userAnnots, int pageNo) {
    ...
    for (size_t i = 0; i < userAnnots.size(); i++) {
        ...
        switch (annot.type) {
            case PageAnnotType::AnnotType_Highlight:
            case PageAnnotType::AnnotType_Underline:
            case PageAnnotType::AnnotType_StrikeOut:
            case PageAnnotType::AnnotType_Squiggly:
                result.Append(annot);
                break;
        }
    }
...
static void fz_run_user_page_annots(Vec<PageAnnotation>& pageAnnots, fz_device* dev, const fz_matrix* ctm,
    ...
    for (size_t i = 0; i < pageAnnots.size() && (!cookie || !cookie->abort); i++) {
        ...
        if (PageAnnotType::AnnotType_Highlight == annot.type) {
...
static void fz_run_page_transparency(Vec<PageAnnotation>& pageAnnots, fz_device* dev, const fz_rect* cliprect,
    ...
    for (size_t i = 0; i < pageAnnots.size(); i++) {
        if (PageAnnotType::AnnotType_Highlight == pageAnnots.at(i).type) {
...
class PdfEngineImpl : public BaseEngine {
  ...
  public:
    ...
    bool AllowsPrinting() const override { return pdf_has_permission(_doc, PDF_PERM_PRINT)>0; }
    bool AllowsCopyingText() const override { return pdf_has_permission(_doc, PDF_PERM_COPY)>0; }
  ...
  protected:
	...
	//CHashList<BYTE> hashPageTypeByPageNo;
};
...
class PdfComment : public PageElement {
  ...
  public:
    PdfComment(const WCHAR* content, RectD rect, int pageNo)
        : annot(PageAnnotType::AnnotType_None, pageNo, rect, PageAnnotation::Color()), content(str::Dup(content)) {}
...
bool PdfEngineImpl::LoadFromStream(fz_stream* stm, PasswordUI* pwdUI) {
    ...
    isProtected = pdf_needs_password(_doc)>0;
    if (!isProtected)
        return true;
...
public:
	...
	BYTE GetPageType(){
		if(pdf_path_count>0&&pdf_image_count==0)
			return BaseEngine::PAGE_TYPE_PDF_STROKE;
		else if(pdf_path_count==0&&pdf_image_count>0)
			return BaseEngine::PAGE_TYPE_PDF_IMAGE;
		else if(pdf_image_count>0&&pdf_image_count>0)
			return BaseEngine::PAGE_TYPE_PDF_IMAGE_AND_STROKE;
		else //if(pdf_image_count==0&&pdf_path_count==0)
			return BaseEngine::PAGE_TYPE_UNINITIALIZED;
	}
};
extern "C" static void fz_check_page_type_stroke_path(fz_device* dev, fz_path* path, fz_stroke_state* stroke,
...
extern "C" static void fz_check_page_type_fill_image(fz_device* dev, fz_image* image, const fz_matrix* ctm, float alpha) {
	UNUSED(alpha);
	// extract rectangles for images a user might want to extract
	// TODO: try to better distinguish images a user might actually want to extract
	if (image->w < 16 || image->h < 16)
		return;
	else if(dev!=NULL&&image!=NULL)
		pdf_image_count++;
}

static fz_device* fz_new_check_page_type_device(fz_context* ctx,ListInspectionData* data) {
	fz_device* dev = fz_new_device(ctx,NULL);
	dev->free_user = NULL;

	dev->fill_path = NULL;
	dev->stroke_path = fz_check_page_type_stroke_path;
	dev->clip_path = NULL;
	dev->clip_stroke_path = NULL;

	dev->fill_shade = NULL;
	dev->fill_image = fz_check_page_type_fill_image;
...
BYTE PdfEngineImpl::GetPageType(int pageNo){
	pdf_page* page = GetPdfPage(pageNo);
	if (!page || !pdf_is_dict(page->me))
		return PAGE_TYPE_UNINITIALIZED;
	//BYTE *pPageType=hashPageTypeByPageNo.GetValue(pageNo);
	//if(pPageType)
	//	return *pPageType;
	//else
	{	//
		...
		EnterCriticalSection(&ctxAccess);
		fz_try(ctx) { dev = fz_new_check_page_type_device(ctx, NULL); }
		fz_catch(ctx) {
			LeaveCriticalSection(&ctxAccess);
			return NULL;
		}
...
RenderedBitmapEx* PdfEngineImpl::RenderBitmapEx(int pageNo, float zoom, int rotation, RectD* pageRect, RenderTarget target,AbortCookie** cookie_out) 
{
	pdf_page* page = GetPdfPage(pageNo);
	if (!page || !pdf_is_dict(page->me))
		return nullptr;
	...
	RenderedBitmapEx* bitmap = nullptr;
	if (ok)
		bitmap = new_rendered_fz_pixmap_ex(ctx, image);
	fz_drop_pixmap(ctx, image);
	return bitmap;
}
...
>>";
    ...
    const char* subtype = PageAnnotType::AnnotType_Highlight == annot.type
                              ? "Highlight"
                              : PageAnnotType::AnnotType_Underline == annot.type
                                    ? "Underline"
                                    : PageAnnotType::AnnotType_StrikeOut == annot.type
                                          ? "StrikeOut"
                                          : PageAnnotType::AnnotType_Squiggly == annot.type ? "Squiggly" : nullptr;
    CrashIf(!subtype);
    ...
    fz_try(ctx) {
        ...
        if (annot.type != PageAnnotType::AnnotType_Highlight)
...
{
	return (double)fz_set_draw_min_line_weight(min_weight);
}
double GetPdfStrokeLineMinWeight()
{
	return (double)fz_get_draw_min_line_weight();
}
double SetPdfStrokeLineMinFlatness(double min_flatness)
{
	return (double)fz_set_draw_min_line_flatness(min_flatness);
}
double GetPdfStrokeLineMinFlatness()
{
	return (double)fz_get_draw_min_line_flatness();
}
double SetPdfStrokeLineMaxWeight(double max_weight)
{
	return (double)fz_set_draw_max_line_weight(max_weight);
}
double GetPdfStrokeLineMaxWeight()
{
	return (double)fz_get_draw_max_line_weight();
}
double SetPdfStrokeLineMaxFlatness(double max_flatness)
{
	return (double)fz_set_draw_max_line_flatness(max_flatness);
}
double GetPdfStrokeLineMaxFlatness()
{
	return (double)fz_get_draw_max_line_flatness();
}
} // namespace PdfEngine
...
class XpsEngineImpl : public BaseEngine {
  ...
  public:
    ...
    RectD PageMediabox(int pageNo) override;
    RectD PageContentBox(int pageNo, RenderTarget target = RenderTarget::View) override;
	BYTE GetPageType(int pageNo){return PAGE_TYPE_UNINITIALIZED;}
    RenderedBitmap* RenderBitmap(int pageNo, float zoom, int rotation,
                                 RectD* pageRect = nullptr, /* if nullptr: defaults to the page's mediabox */
                                 RenderTarget target = RenderTarget::View, AbortCookie** cookie_out = nullptr) override;
	RenderedBitmapEx* RenderBitmapEx(int pageNo, float zoom, int rotation,
...
RenderedBitmapEx* XpsEngineImpl::RenderBitmapEx(int pageNo, float zoom, int rotation, RectD* pageRect, RenderTarget target,AbortCookie** cookie_out) 
{
	UNUSED(target);
	xps_page* page = GetXpsPage(pageNo);
	if (!page)
...
bool XpsEngineImpl::SaveFileAs(const char* copyFileName, bool includeUserAnnots) {
    ...
    if (!FileName())
        return false;
    return CopyFileW(FileName(), dstPath, FALSE)!=0;
}
