...
bool LaunchFileFunc(const WCHAR* path, const WCHAR* params, const WCHAR* verb, bool hidden) {
    if (!path)
        return false;
...
BitmapPixels* GetBitmapPixels(HBITMAP hbmp) {
	...
	if (IsPalettedBitmap(info, nBytes)) {
		CrashIf(info.dsBmih.biBitCount != 8);
		 HANDLE hOld=SelectObject(hdc, hbmp);
		// color order in DIB is blue-green-red-alpha
		res->nPalette = GetDIBColorTable(hdc, 0, dimof(res->palette), res->palette);	
...
int GetBitmapPixelsAndPalette(HBITMAP hbmp,BYTE* bmpData,int nWidth,int nHeight,int biBitCount,RGBQUAD *pPalette,int paletteCount) 
{
	DWORD bmpBytes = 0;
	if(bmpData==NULL)
	{
		SizeI size=GetBitmapSize(hbmp);
		bmpBytes = ((size.dx * 3 + 3) / 4) * 4 * size.dy;
		return bmpBytes;
	}
	else
		bmpBytes = ((nWidth * 3 + 3) / 4) * 4 * nHeight;
	...
	CrashIf(!bmpData);
	//获取调色板信息
	if (biBitCount!=0&&biBitCount<=8&&pPalette) {
		HANDLE hOld=SelectObject(hdc, hbmp);
		// color order in DIB is blue-green-red-alpha
		int nPalette = GetDIBColorTable(hdc, 0, paletteCount, pPalette);	
		SelectObject(hdc,hOld);
	}

	GetDIBits(hdc, hbmp, 0, nHeight, bmpData, &bmi, DIB_RGB_COLORS);
	DeleteDC(hdc);
	return bmpBytes;
}
