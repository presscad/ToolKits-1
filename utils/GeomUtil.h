...
class RectT {
  public:
	...
	static RectT FromPoint(PointT<T> TL,PointT<T> TR,PointT<T> BL,PointT<T> BR){
		T minX=min(TL.x,BL.x);
		T minY=min(TL.y,TR.y);
		T maxX=max(BR.x,TR.x);
		T maxY=max(BL.y,BR.y);
		return FromXY(minX,minY,maxX,maxY);
	}
    template <typename S>
...
#if 0 // def GDIPVER, note: GDIPVER not defined in mingw?
