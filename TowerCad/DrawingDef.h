...
#define FIND_BYDXFHANDLE		0x0800
...
//**************************************************************
typedef struct tag_OBJXRECORD {
	HANDLE handle;
	int nValCount;
	RES_BUF resValArr[100];
}OBJXRECORD, *POBJXRECORD;

typedef struct tag_OBJDICTIONARY{
	HANDLE handle;
	int nRecCount;
	DICTIONARY_REC recArr[100];
}OBJDICTIONARY,*POBJDICTIONARY;

typedef struct tag_OBJECTHEADER
{
	unsigned short	ObjType;			// Obj type
	OBJHANDLE	Objhandle;				// Handle 
	HANDLE		DxfObjHandle;			// Dxf Object Handle
	BOOL		Deleted;				// 0 = Not Deleted
	DWORD		PreviousObjPos;
	DWORD		NextObjPos;
} OBJECTHEADER, *POBJECTHEADER;

typedef struct tag_OBJECTS
{
	HGLOBAL		hObjs;
	DWORD		TotalSize;
	DWORD		FreePos;
