...
CDrawing::CDrawing()
{
	...
	// OBJECTS
	result &= ((drwAddObject = (OBJHANDLE(*)(HDRAWING, OBJHANDLE, POBJECTHEADER, LPVOID))
		GetProcAddress( hinstCadIO, "drwAddObject" )) != NULL);
	result &= ((drwDeleteObject = (BOOL(*)(HDRAWING, LPCTSTR, OBJHANDLE))
		GetProcAddress( hinstCadIO, "drwDeleteObject" )) != NULL);
	result &= ((drwChangeObject = (BOOL(*)(HDRAWING, LPCTSTR, POBJECTHEADER, LPVOID))
		GetProcAddress( hinstCadIO, "drwChangeObject" )) != NULL);
	result &= ((drwFindObject = (OBJHANDLE(*)(HDRAWING, LPCTSTR, POBJECTHEADER, LPVOID, DWORD))
		GetProcAddress( hinstCadIO, "drwFindObject" )) != NULL);
...
OBJHANDLE CDrawing::MText( LPCTSTR Text, double x1, double y1, double x2, double y2, double Height, 
				 short Justification, double Width, short FlowDirection,double Rotation/*=0*/, double WidthFactor/*=0*/)
{
	ENTTEXT text;
	text.Point0.x = x1;
	text.Point0.y = y1;
	...
	text.TextData.SecondAlignmentPoint.y = y2;
	text.TextData.TextStyleObjhandle = CurrentStyleObjhandle;
	text.TextData.FlowDirection = FlowDirection;
	text.TextData.Width = Width;
	m_EntityHeader.EntityType = ENT_MTEXT;
	return AddEntity(ActiveBlockObjhandle, &m_EntityHeader, &text);
}
...
// OBJECTS --------------------------------------------------------
OBJHANDLE CDrawing::AddObject( OBJHANDLE BlockObjhandle, POBJECTHEADER pObjectHeader, LPVOID pObjectData )
{
	ISLIBRARYLOADED();
	return drwAddObject(m_hDrawing, BlockObjhandle, pObjectHeader, pObjectData);
}

BOOL CDrawing::DeleteObject( LPCTSTR strBlockName, OBJHANDLE Objhandle )
{
	ISLIBRARYLOADED();
	return drwDeleteObject(m_hDrawing, strBlockName, Objhandle);
}

BOOL CDrawing::ChangeObject( POBJECTHEADER pObjectHeader, LPVOID pObjectData, LPCTSTR strBlockName )
{
	ISLIBRARYLOADED();
	return drwChangeObject(m_hDrawing, strBlockName, pObjectHeader, pObjectData);
}

OBJHANDLE CDrawing::FindObject( DWORD dwFindType, POBJECTHEADER pObjectHeader, LPVOID pObjectData, LPCTSTR strBlockName )
{
	ISLIBRARYLOADED();
	return drwFindObject(m_hDrawing, strBlockName, pObjectHeader, pObjectData, dwFindType);
}
