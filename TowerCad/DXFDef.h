...
typedef struct tag_DXFTEXTDATA {
	...
	short		FlowDirection;		//MTEXT启用，字体方向
	double		Width;				//MTEXT启用，一行最大宽度
...
// OBJECTS wht 15-10-10
typedef struct tag_DXFOBJECTHEADER{
	unsigned short	ObjectType;			// Object type
} DXFOBJECTHEADER, *PDXFOBJECTHEADER;
union RES_U_VAL {
    double rreal;
    double rpoint[3];
    short rint;
 ...
 */
typedef struct tag_RES_BUF {                                             
   short restype;
   union RES_U_VAL resval;
   tag_RES_BUF(){memset(this,0,sizeof(tag_RES_BUF));}
}RES_BUF;
typedef struct tag_DICTIONARY_REC{
	char sName[20];		//词典条目名称
	HANDLE hRec;		//词典条目对应对象句柄
	tag_DICTIONARY_REC(){memset(this,0,sizeof(tag_DICTIONARY_REC));}
}DICTIONARY_REC;
typedef struct tag_DXFOBJXRECORD {
	HANDLE handle;
	int nValCount;
	RES_BUF resValArr[50];
}DXFOBJXRECORD, *PDXFOBJXRECORD;
typedef struct tag_DXFOBJDICTIONARY{
	HANDLE handle;
	int nRecCount;
	DICTIONARY_REC recArr[50];
}DXFOBJDICTIONARY,*PDXFOBJDICTIONARY;
// Arrow Heads --------
struct ARROWHEAD
{
